/* ============================================================
   OLOGENETICA · I18N — lingua + selettore (in alto)
   Lingue: it, en, fr, es, pt (BR).
   Persistenza: URL ?lang=xx  >  localStorage  >  lingua browser.
   API: ZI18N.get()/set(code)/t(dict)/pickerHTML()/mount(el)/decorate()
   Evento window 'zlangchange'.
   (Adattato da zodiac-zine.)
   ============================================================ */
(function () {
  const LANGS = [
    ...
    { code: "pt", label: "BR", name: "Português (BR)" }
  ];
  const KEY = "hlang";
  ...
  function injectCSS() {
    ...
    st.textContent = `
.zlang-picker{display:inline-flex;gap:3px;background:#fff;border:2.5px solid #161125;border-radius:999px;padding:3px;box-shadow:2px 2px 0 #161125}
.zlang-btn{font-family:'Space Mono',monospace;font-weight:700;font-size:11px;border:none;background:transparent;color:#161125;padding:4px 8px;border-radius:999px;cursor:pointer;line-height:1}
.zlang-btn:hover{background:#ffe9a8}
.zlang-btn.on{background:#161125;color:#ffd23f}`;
  ...
  // PWA: lo strumento funziona anche offline
